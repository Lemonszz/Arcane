...
import party.lemons.magicmod.client.SpellUtilClient;
...
public class GuiSpellBook extends GuiScreen
{
	...
	protected void actionPerformed(GuiButton button) throws IOException
	{
		...
		if(button.id == storeLevelButton.id)
		{
			if(mc.player.experienceLevel >= 10)
			{
				...
				ModNetwork.NETWORK_INSTANCE.sendToServer(new PacketSendAddPoint());
			}
		}
		...
		if(button.id == increaseManaButton.id)
		{
			if(mc.player.experienceLevel >= 10)
			{
				...
				ModNetwork.NETWORK_INSTANCE.sendToServer(new PacketSendAddMaxMana());
			}
		}
						...
						else
						{
							if(data.hasSpellUnlocked(sp))
								...
								if(data.getStoredLevels() >= sp.getUnlockCost())
								{
									...
									ModNetwork.NETWORK_INSTANCE.sendToServer(new PacketSendUnlockSpell(sp.getRegistryName().toString()));
								}
							break;
						}
					}
			...
			if(mouseX >= xx && mouseX <= xx + width)
			{
				if(mouseY >= yy && mouseY <= yy + height)
				{
					...
					else
					{
						...
						ModNetwork.NETWORK_INSTANCE.sendToServer(new PacketSendSelectedSpell(b));
					}
				}
			}
			yy += gapSize;
	...
	public void onGuiClosed()
	{
		for(int i = 0; i < selectedSpells.length; i++)
		{
			...
			{
				ModNetwork.NETWORK_INSTANCE.sendToServer(new PacketSendChosenSpell(selectedSpells[i].getRegistryName().toString(), i));
			}
			else
			{
				ModNetwork.NETWORK_INSTANCE.sendToServer(new PacketSendChosenNullSpell(i));
			}
		}
		super.onGuiClosed();
	}
