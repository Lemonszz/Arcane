...
import party.lemons.magicmod.network.PacketSendSelectedSpell;
...
public class GuiRadialMenu extends GuiScreen
{
			...
			for(int i = 0; i < 6; i++)
			{
				...
				if(mouseX >= xx && mouseX <= xx + 18)
				{
					if(mouseY >= yy && mouseY <=yy + 18)
					{
						...
						ModNetwork.NETWORK_INSTANCE.sendToServer(new PacketSendSelectedSpell(i));
					}
				}
	...
	private static final ResourceLocation _guiTexture = new ResourceLocation(ModConstants.MODID ,"textures/gui/spellbook.png");
