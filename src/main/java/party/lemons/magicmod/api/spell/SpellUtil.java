...
public class SpellUtil
{
	public static void castSpellPress(Spell spell, EntityPlayer player)
	{
		...
		if(!player.world.isRemote)
			ModNetwork.NETWORK_INSTANCE.sendTo(new PacketSyncCurrentMana(player.getCapability(PlayerData.CAPABILITY, null).getMana()), (EntityPlayerMP) player);
	}
	...
	public static void syncData(EntityPlayer player)
	{
		...
		{
			ModNetwork.NETWORK_INSTANCE.sendTo(new PacketUnlockSpell(sp.getRegistryName().toString()), pl);
		}
		for(int i = 0; i < newData.getSelectedSpells().length; i++)
		{
			...
			if(sp != null)
			{
				...
				ModNetwork.NETWORK_INSTANCE.sendTo(new PacketSyncSelectedSpell(sp.getRegistryName().toString(), i), pl);
			}
		}
		ModNetwork.NETWORK_INSTANCE.sendTo(new PacketSyncSelectedIndex(newData.getSelectedIndex()), pl);
		ModNetwork.NETWORK_INSTANCE.sendTo(new PacketSyncMaxMana(newData.getMaxMana()), pl);
		ModNetwork.NETWORK_INSTANCE.sendTo(new PacketSyncCurrentMana(newData.getMana()), pl);
		ModNetwork.NETWORK_INSTANCE.sendTo(new PacketSyncStoredLevels(newData.getStoredLevels()), pl);

		//ModNetwork.NETWORK_INSTANCE.sendTo(new PacketClientDebugData(), pl);
	}
